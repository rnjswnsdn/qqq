...
function handleDeviceMotion(event) {
    ...
    if (accelerationMagnitude > vibrationThreshold) {
        ...
        vibrationData.push({ value: accelerationMagnitude, time: currentTime });
        addToVibrationDataList(accelerationMagnitude, currentTime);
        updateVibration(accelerationMagnitude, currentTime); // 진동 값 갱신
    }
...
function drawVibrationGraph() {
    ...
    const maxVibration = Math.max(...vibrationData.map(data => data.value));
    ...
    for (let i = 0; i < vibrationData.length; i++) {
        const barHeight = (vibrationData[i].value / maxVibration) * height;
...
function drawDBGraph() {
    ...
    if (dB >= 38) {
        let currentTime = new Date().toLocaleTimeString();
        addToDBDataList(dB, currentTime);
        updateDb(dB, currentTime); // 데시벨 값 갱신
    }
...
// 데시벨 값 목록에 추가
function addToDBDataList(value, time) {
    const listItem = document.createElement('li');
    listItem.textContent = `데시벨: ${value.toFixed(2)} dB, 시간: ${time}`;
    dbDataList.appendChild(listItem);
}

// 진동 값 목록에 추가
function addToVibrationDataList(value, time) {
    const listItem = document.createElement('li');
    listItem.textContent = `진동: ${value.toFixed(2)}, 시간: ${time}`;
    vibrationDataList.appendChild(listItem);
}

// 진동 값 목록에 업데이트
function updateVibration(value, time) {
    vibrationData.push({ value, time });
    localStorage.setItem('vibrationData', JSON.stringify(vibrationData));  // 진동 값 저장
    updateVibrationList();
}

// 데시벨 값 목록에 업데이트
function updateDb(value, time) {
    dbData.push({ value, time });
    localStorage.setItem('dbData', JSON.stringify(dbData));  // 데시벨 값 저장
    updateDbList();
}
...
function updateVibrationList() {
    ...
    vibrationList.innerHTML = '';  // 기존 목록 초기화
    vibrationData.forEach(item => {
        const li = document.createElement('li');
        li.textContent = `진동 세기: ${item.value}, 시간: ${item.time}`;
...
function updateDbList() {
    ...
    dbData.forEach(item => {
        const li = document.createElement('li');
        li.textContent = `데시벨: ${item.value} dB, 시간: ${item.time}`;
...
window.onload = function () {
    ...
    if (storedVibrationData) {
        vibrationData = storedVibrationData;
        updateVibrationList();
    }
    if (storedDbData) {
        dbData = storedDbData;
